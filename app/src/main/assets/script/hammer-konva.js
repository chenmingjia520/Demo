...
(function (window, document, exportName, undefined) {
    ...
    function inputHandler(manager, eventType, input) {
        ...
        // 源事件是domEvents的规范化值
        // 比如“touchstart，mouseup，pointerdown”
     ...
     *触摸和鼠标组合输入
     *
     *触摸具有比鼠标更高的优先级，并且在触摸时不允许鼠标事件。
     *这是因为触摸设备在进行触摸时也会发出鼠标事件。
     *
     * @constructor
     * @extends Input
     */
    ...
    inherit(PressRecognizer, Recognizer, {
        ...
        defaults: {
            event: 'press',
            pointers: 1,
            time: 800, // minimal time of the pointer to be pressed
    ...
    Manager.prototype = {
        ...
        /**
         *运行识别器！
         *在指针的每次移动（触碰）时由inputHandler函数调用
         *它遍历所有的识别器并尝试检测所做的手势
         * @param {Object} inputData
         */
        recognize: function (inputData) {
            ...
            //它保存正在被识别的识别器。
            //因此，识别器的状态需要开始、更改、结束或识别
            //如果没有识别器检测到某个对象，则将其设置为“null”`
            ...
            while (i < recognizers.length) {
                ...
                //找出是否允许我们尝试识别这个输入。
                //1。如果会话未强制停止，则允许（请参见.stop（）方法）
                //2。如果我们在此会话中仍然没有识别出手势，或者此识别器就是该手势，则允许
                //这是公认的。
                //3。如果允许识别器与当前识别器同时运行，则允许。
                //这可以使用识别器上的“recognizeWith（）”方法进行设置
                if (
                ...
                //如果识别器已经将输入识别为有效的手势，我们希望将此作为
                //当前活动的识别器。但前提是我们还没有激活的识别器
                if (
                    !curRecognizer &&
